...
class CheckIOSolver:
    ...
    def try_code(self):
        ...
        unsolved_islands_tasks = self.get_unsolved_tasks_from_island("https://py.checkio.org/station/library/")
        self.solve_tasks_on_island(unsolved_islands_tasks)
        time.sleep(3)
        self.driver.quit()
    ...
    def copy_solution(self, solution_link):
        ...
        try:
            ...
            print(e)
            print("Can't get solution from: ", solution_link)
            return ''
    ...
    def get_google_solutions_for_task(self, task):
        ...
        current_google_results_links = []
        for results in google_results:
            current_google_results_links.append(results.get_attribute("href"))
        time.sleep(3)
        return current_google_results_links
    ...
    def get_island_links(self):
        status = self.driver.find_elements_by_xpath("//div[contains(@class, 'map__station_state_opened')]")
        opened_islands = []
        for each in status:
            opened_islands.append(each.find_element_by_css_selector('a.map__station__link').get_attribute("href"))
        return opened_islands
